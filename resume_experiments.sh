...
for i in 1 2 3 4; do
  if [ -f "results/mgcnn_sdtransformer_seed${i}.json" ]; then
...
for i in 0 1 2 3 4; do
  if [ -f "results/multiseed/dualpq_concat_noaug_seed${i}.json" ]; then
    echo "Seed $i already completed. Skipping."
    continue
  fi
...
echo "=== OPEN-6 & OPEN-7: Regenerate summaries and stats ==="
